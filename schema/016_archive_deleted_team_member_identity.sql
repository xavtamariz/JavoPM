-- Archive deleted member identities so owners can reuse the same name/nickname.
...
begin
  ...
  set
    name = current_row.name || ' (eliminado ' || current_row.id || ')',
    nickname = null,
    status = 'inactive',
  ...
  values (
    ...
    p_actor_user_id,
    'member_deactivated',
    jsonb_build_object('deleted', true, 'nickname', current_row.nickname, 'name', current_row.name),
    p_client_id
  );
...
set
  name = case
    when position('(eliminado ' || id || ')' in name) = 0 then name || ' (eliminado ' || id || ')'
    else name
  end,
  nickname = null
